...
def snowman(snowman_word):
  ...
  # progress = get_word_progress(snowman_word, snowman_word_dict)
  num_wrong_guesses = 0
  
  while len(wrong_guesses_list) < SNOWMAN_MAX_WRONG_GUESSES and not all(x==True for x in snowman_word_dict.values()):

    user_input_string = get_letter_from_user(snowman_word_dict, wrong_guesses_list)
    if user_input_string in snowman_word_dict:
      print("You guessed a letter that's in the word!")
      snowman_word_dict[user_input_string] = True
      print (snowman_word_dict)
      get_word_progress(snowman_word, snowman_word_dict)
  ...
  if all(snowman_word_dict.values()):
